    ...
    public partial class BiletSatis : Form
    {
        ...
        private void seansGetir()
        {
            try
            {
                ...
                // Seçili filme göre seansları alacağımız komut.
                ...
                while (verioku.Read())
                {
                    // geçerli bilgisayar saat ve tarih bilgilerinden bugün yani süresi dolmamış olan seans bilgilerini getirecektir.
                    ...
                    else if (DateTime.Parse(verioku["tarih"].ToString()).Day > DateTime.Now.Day && DateTime.Parse(verioku["tarih"].ToString()).Month == DateTime.Now.Month && DateTime.Parse(verioku["tarih"].ToString()).Year >= DateTime.Now.Year && DateTime.Parse(verioku["saat"].ToString()).Hour >= DateTime.Now.Hour)
                    {
                        if ((DateTime.Parse(verioku["saat"].ToString()).Minute >= DateTime.Now.Minute && DateTime.Parse(verioku["saat"].ToString()).Hour == DateTime.Now.Hour) || (DateTime.Parse(verioku["saat"].ToString()).Hour >= DateTime.Now.Hour))
                            cmbx_seans.Items.Add(verioku["salon"].ToString() + " " + verioku["tarih"].ToString() + " " + verioku["saat"].ToString());
                    }
                    ...
                    else if (DateTime.Parse(verioku["tarih"].ToString()).Month > DateTime.Now.Month && DateTime.Parse(verioku["tarih"].ToString()).Year >= DateTime.Now.Year)
                        cmbx_seans.Items.Add(verioku["salon"].ToString() + " " + verioku["tarih"].ToString() + " " + verioku["saat"].ToString());
                }
                verioku.Close();
        ...
        private void button_kaydet_Click(object sender, EventArgs e)
        {
            if (txt_ad.Text != "" && txt_mail.Text != "" && cmbx_film.Text != "" && cmbx_seans.Text != "" && cmbx_koltukno.Text != "" && txt_salon.Text != "")
            {
                ...
                {
                    Form_Vizyon.baglantiac();
                    SqlCommand kaydet = new SqlCommand("INSERT INTO Biletler (film,seans,salon,koltukno,ad,mail)VALUES('" + cmbx_film.Text + "','" + cmbx_seans.Text + "','" + txt_salon.Text + "','" + cmbx_koltukno.Text + "','" + txt_ad.Text + "','" + txt_mail.Text + "')", Form_Vizyon.baglanti);
                    kaydet.ExecuteNonQuery();
                    kaydet = new SqlCommand("INSERT INTO BiletGecmisleri (baslik,aciklama,saat,tarih)VALUES('Bilet satış','" + cmbx_film.Text + " adlı filmin " + cmbx_seans.Text + " tarihinde olan seansına " + txt_ad.Text + " adlı müşteriye bilet satışı yapıldı.','" + DateTime.Now.ToShortTimeString() + "','" + DateTime.Now.ToShortDateString() + "')", Form_Vizyon.baglanti);
                    kaydet.ExecuteNonQuery();
                    Form_Vizyon.baglantikapa();
                    temizle();
